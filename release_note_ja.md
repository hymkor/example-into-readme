...
- 見出し中の Markdown リンク (`[text](url)`) によりアウトラインが壊れる問題を修正 (#6)
- 標準出力だけを引用する `stdout: コマンド`、標準出力・エラー出力ともに引用する `output:` コマンドを追加 (#7)
- cmd/badges: `github repo` badge を追加 (#8)

v0.8.2
------
Feb 3, 2026

- 対象ファイルを省略した場合、`readme.md` など `README.md` と大文字・小文字のみ異なる名前のファイルが存在すると、更新後のファイル名が一律 `README.md` になってしまう問題を修正した。(#3)
